import { IProductImage } from '../interfaces/tbl_product.interface';
...
export class ProductImageService {
  ...
  constructor() {
    ...
    this.tableName = 'tbl_product_image';
  }

  public create(data: Partial<IProductImage>) {
    return new Promise(async (resolve, reject) => {
      try {
        const connection = await this.pool.getConnection();
        await connection
          .request()
          .input('url', data.primg_url)
          .input('prodId', data.prod_id)
          .query(
            `INSERT INTO ${this.tableName}(primg_url, prod_id)
              VALUES (@url, @prodId);
  ...
  public bulkCreate(data: Partial<IProductImage>[]) {
    return new Promise(async (resolve, reject) => {
      try {
        const query = `
          SELECT
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await connection
          .request()
          .input('id', id)
          .query(`SELECT * FROM ${this.tableName} WHERE primg_id = @id`)
          .then((response) => {
            resolve({ data: response.recordset });
            connection.close();
          })
  ...
  public update(id: number, data: Partial<IProductImage>) {
    return new Promise(async (resolve, reject) => {
      try {
        const connection = await this.pool.getConnection();
        await connection
          ...
          .query(
            `UPDATE ${this.tableName}
              SET primg_url = @url, prod_id = @prodId, primg_updated_at = @date
              WHERE primg_id = @id`
          )
          .then(() => {
            resolve({ result: 'Done' });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await connection
          ...
          .query(
            `UPDATE ${this.tableName}
              SET primg_is_valid = @isValid, primg_updated_at = @date, primg_deleted_at = @date
              WHERE primg_id = @id`
          )
          .then(() => {
            resolve({ result: 'Done' });
